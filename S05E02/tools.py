...
# Głos „Adam” (premade) — API wymaga voice_id, nie nazwy.
...
def call_verify_api(**kwargs) -> str:
    ...
    if answer_payload.get("action") == "start":
        logger.info("[AKCJA] Wykryto akcję 'start'. Wysyłam bezpośrednio bez przetwarzania audio.")
    ...
    # Używamy elif, aby ten blok nie wykonał się, gdy akcją jest 'start'
        ...
        try:
            if not tts_client:
                raise RuntimeError("Klient ElevenLabs nie został zainicjalizowany (brak ELEVENLABS_API_KEY?).")
            # SDK elevenlabs>=2: text_to_speech.convert zwraca Iterator[bytes]
            audio_bytes = b"".join(
            ...
            with open(agent_audio_path, "wb") as f:
                ...
                f.write(audio_bytes)
            logger.info(f"[ZAPIS AUDIO] Zapisano wypowiedź Agenta do pliku: {agent_audio_path.name}")

            # answer_payload["audio"] = base64.b64encode(tts_response.content).decode('utf-8')
    ...
    try:
        ...
        return json.dumps({"error": f"Błąd API lub parsowania: {e}"}, ensure_ascii=False)
    ...
    if data.get("code", 0) < 0:
        ...
        logger.warning(f"Rozmowa spalona! Otrzymano kod błędu {error_code}: {error_message}")
        return json.dumps({
    ...
    for key in ["message", "reply", "audio"]:
        if key in data and isinstance(data[key], str) and len(data[key]) > 200:
            ...
            try:
                ...
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                operator_audio_path = settings.logs_dir_path / f"{timestamp}_operator.mp3"
                
                with open(operator_audio_path, "wb") as f:
                    f.write(audio_bytes)
                logger.info(f"[ZAPIS AUDIO] Zapisano odpowiedź Operatora do pliku: {operator_audio_path.name}")
                ...
                logger.info(f"[Whisper] Operator powiedział: {transcript.text}")
                data[key] = f"(Transkrypcja nagrania od operatora): {transcript.text}"
            except Exception as e:
                logger.debug(f"Nie udało się odkodować Base64 (to najpewniej czysty tekst): {e}")
