...
def main():
    ...
    if not api_key or not model_id or not hub_url or not openrouter_url:
        raise RuntimeError(
            "Brak jednej z wymaganych zmiennych środowiskowych: \
            OPENROUTER_API_KEY / MODEL_ID / HUB_URL / OPENROUTER_URL"
        )
    if not hub_api_key:
        raise RuntimeError(
    ...
    # 2. Przefiltruj dane
    out_path = Path(__file__).parent / "data" / "people_filtered.csv"
    if not os.path.exists(out_path):
        in_path = Path(__file__).parent / "data" / "people.csv"
    ...
    tagged_path = Path(__file__).parent / "data" / "people_tagged.csv"
    tags = TAGS
    ...
    transport_path = Path(__file__).parent / "data" / "people_transport.csv"
